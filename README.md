...
- Chi tiết về Khung toán học định mức, thiết kế quy trình chấm điểm và các nguồn lực nghiên cứu (bao gồm tập dữ liệu và mô hình Ollama) được lưu trữ tại [Tài liệu Tham khảo](file:///f:/data_project/reports/docs/tham_khao.md).
...
- **Kiểm soát tệp tin cấu hình (`.gitignore`)**:
  - Tệp [.gitignore](file:///f:/data_project/.gitignore) đã được cấu hình chặt chẽ để loại bỏ hoàn toàn các tệp cơ sở dữ liệu nội bộ (`data/raw/*.db`, `data/**/*.db`), ma trận đặc trưng trung gian (`data/processed/`), các tệp mô hình đã huấn luyện (`models/*.pkl`), và báo cáo chi tiết (`data/outputs/`).
